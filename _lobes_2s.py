...
def advertised_lift_duration(x: np.array, y: np.array) -> tuple[float, float]:
    step = x[1] - x[0]
    first_lift_index = np.argmax(y > 0.000001)
    last_lift_index = len(y) - np.argmax(y[::-1] > 0.000001) - 1
...
def update_lobe_function(content, x, y, base, lobe_name, header):
    def create_samples(x, y, step):
        ...
        for i, deg in enumerate(x):
            ...
            if not np.isclose(y[i], 0, atol=0.001):
                ...
                r2 = deg
                res2 += f"\n        .add_sample({deg:.3f} * units.deg, {y[i]:.3f} * units.mm)"
        res2 = f"\n        .add_sample({l2 - step:.3f} * units.deg, 0.000 * units.mm)" + res2 + f"\n        .add_sample({r2 + step:.3f} * units.deg, 0.000 * units.mm)"
    ...
    function _lobe_profile({step:.3f} * units.deg)
        _lobe_profile{create_samples(x / 2, y, step)}\n}}\n""" # lobes diff
...
def create_profile(duration, lift, trim_factor, sigma, vol, res, at_lift, ramp_steepness, ramp_pos, lift_significant_fraction, cos):
    ...
    x = mult(min_max_scale_array(x) - 1, duration / 2) # lobes diff
...
def plot_all(ax, fpath, cfg, int_lift, exh_lift, ivo, ivc, evo, evc):
    ...
    i_vly = create_valve_path_roller_follower(i_x, i_y, i_base, 0.001)
    ...
    advance_line = np.deg2rad(0 - i_c + e_c)
...
def plot(ax, x, y, vly, angle, base, dur_at_lift, rtr, name, at_lift, color):
    ...
    min_l, min_r = advertised_lift_duration(x, vly)
    ...
    if at_lift != 0:
        ax.plot([0, np.deg2rad(dur_at_lift / 2 + angle)], [0, base + at_lift], linestyle='-', color=color, linewidth=0.5, label=f'{name} duration @ {at_lift:.3f} mm') # lobe @ lift start
        ax.plot([0, np.deg2rad(-dur_at_lift / 2 + angle)], [0, base + at_lift], linestyle='-', color=color, linewidth=0.5) # lobe @ lift end
    legend_angle = -45
